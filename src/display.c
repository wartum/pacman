...
void Display_register_map(Tile *map, uint32_t width, uint32_t height, uint32_t scale) {
    ...
    int origin_x = (SCREEN_W / 2) - ((scale * display.map_w) / 2);
    int origin_y = (SCREEN_H / 2) - ((scale * display.map_h) / 2);
    for (uint32_t i = 0; i < display.map_w; i++) {
        for (uint32_t j = 0; j < display.map_h; j++) {
            Tile *tile = get_tile(i, j);
            tile->rectangle.w = scale - 5;
            tile->rectangle.h = scale - 5;
            tile->rectangle.x = (scale * i) + origin_x;
            tile->rectangle.y = (scale * j) + origin_y;
        }
    }
}
